...
from cashier.models import Venta, VentaDetalle, VentaPago
import hashlib
import calendar
...
def compute_analytics(fecha_inicio, fecha_fin, cajero_filter='todos', sucursal_filter='todos', limit_rentabilidad=50):
    ...
    ingreso_total = to_clp_pesos(agg_ventas.get('ingreso_total') or Decimal('0.00'))
    ...
    cmv = to_clp_pesos(Decimal(str(cmv_val)))

    ganancia_bruta = to_clp_pesos(ingreso_total - cmv)
    ticket_promedio = to_clp_pesos((ingreso_total / num_transacciones) if num_transacciones > 0 else Decimal('0.00'))
    ...
    # Desglose por forma de pago (soporta pagos mixtos via VentaPago)
    legacy_ventas_qs = ventas_qs.filter(pagos__isnull=True)
    nc_expr = (F('monto_nota_credito'))
    metodo_expr = (F('total') - nc_expr)

    pagos_agg = VentaPago.objects.filter(venta__in=ventas_qs).values('metodo').annotate(total_monto=Sum('monto'))
    totales_por_metodo = {row['metodo']: to_clp_pesos(row.get('total_monto') or Decimal('0.00')) for row in pagos_agg}
    ...
    for row in legacy_by_method:
        ...
        monto = to_clp_pesos(row.get('total_monto') or Decimal('0.00'))
        if metodo:
            totales_por_metodo[metodo] = to_clp_pesos(totales_por_metodo.get(metodo, Decimal('0.00')) + monto)

    legacy_nc_total = legacy_ventas_qs.aggregate(total=Sum(nc_expr)).get('total') or Decimal('0.00')
    totales_por_metodo['nota_credito'] = to_clp_pesos(totales_por_metodo.get('nota_credito', Decimal('0.00')) + legacy_nc_total)
    ...
    for metodo, monto in totales_por_metodo.items():
        if not metodo:
            continue
        sales_by_payment.append({'forma_pago': metodo, 'total_monto_raw': monto})
        sales_by_payment_chart.append({'forma_pago': metodo, 'total_monto': _clp_int(monto)})

    if ingreso_total > 0:
        ingreso_sin_iva = to_clp_pesos((ingreso_total / Decimal('1.19')).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP))
        iva_total_calc = to_clp_pesos(ingreso_total - ingreso_sin_iva)
    else:
        ingreso_sin_iva = Decimal('0.00')
        iva_total_calc = Decimal('0.00')

    cost_net = to_clp_pesos((cmv / Decimal('1.19')).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP) if cmv else Decimal('0.00'))
    ganancia_neta = to_clp_pesos(ingreso_sin_iva - cost_net)
    ...
    while current <= end_date:
        ingreso_dia = to_clp_pesos(ingresos_map.get(current, Decimal('0.00')))
        costo_dia = Decimal(str(detalle_map.get(current, 0)))
        ingreso_sin_iva_dia = to_clp_pesos((ingreso_dia / Decimal('1.19')).quantize(Decimal('0.01')) if ingreso_dia else Decimal('0.00'))
        costo_sin_iva_dia = to_clp_pesos((Decimal(str(costo_dia)) / Decimal('1.19')).quantize(Decimal('0.01')) if costo_dia else Decimal('0.00'))
        ganancia_neta_dia = to_clp_pesos(ingreso_sin_iva_dia - costo_sin_iva_dia)
        day_str = current.strftime('%Y-%m-%d')
        daily_chart.append({'day': day_str, 'ingreso': _clp_int(ingreso_dia), 'ganancia_neta': _clp_int(ganancia_neta_dia)})
    ...
    for suc in Sucursal.objects.all():
        ingreso = to_clp_pesos(ingresos_map.get(suc.id, Decimal('0.00')))
        costo_val = costos_map.get(suc.id, 0)
        costo = Decimal(str(costo_val))
        ingreso_sin_iva_suc = to_clp_pesos((ingreso / Decimal('1.19')).quantize(Decimal('0.01')) if ingreso else Decimal('0.00'))
        costo_sin_iva_suc = to_clp_pesos((Decimal(str(costo)) / Decimal('1.19')).quantize(Decimal('0.01')) if costo else Decimal('0.00'))
        ganancia_neta_suc = to_clp_pesos(ingreso_sin_iva_suc - costo_sin_iva_suc)
        branch_comparison.append({'sucursal': suc.nombre, 'ingreso': _clp_int(ingreso), 'ganancia_neta': _clp_int(ganancia_neta_suc)})

    # Distribución horaria y heatmap (usar agregaciones en DB)
    hourly_distribution = [{'hora': h, 'ventas': 0, 'ingreso': 0} for h in range(24)]
    heatmap_matrix = [[{'ventas': 0, 'ingreso': 0} for _ in range(24)] for _ in range(7)]
    # Agregar por hora
    ...
    for row in hours_qs:
        ...
        if h is not None and 0 <= h < 24:
            ...
            hourly_distribution[h]['ingreso'] = _clp_int(row['ingreso'] or 0)
    ...
    for row in heat_qs:
        ...
        if 0 <= py_wk < 7 and h is not None and 0 <= h < 24:
            ...
            heatmap_matrix[py_wk][h]['ingreso'] = _clp_int(row['ingreso'] or 0)
    ...
    if rentabilidad_productos is None:
        ...
        for det in detalles_rango:
            ...
            venta_sin_iva_unit = to_clp_pesos((venta_unit / Decimal('1.19')).quantize(Decimal('0.01')) if venta_unit else Decimal('0.00'))
            compra_sin_iva_unit = to_clp_pesos((compra_unit / Decimal('1.19')).quantize(Decimal('0.01')) if compra_unit else Decimal('0.00'))
            ganancia_neta_unit = to_clp_pesos(venta_sin_iva_unit - compra_sin_iva_unit)
            entry = tmp.get(pid)
            if not entry:
        ...
        for _, data in tmp.items():
            ...
            rentabilidad_productos.append({
                ...
                'ingreso_neto_total': _clp_int(data['ingreso_neto_total']),
                'costo_neto_total': _clp_int(data['costo_neto_total']),
                'ganancia_neta_total': _clp_int(data['ganancia_neta_total']),
    ...
    for _, data in ranking_cajeros_map.items():
        ticket_prom = to_clp_pesos((data['ingreso_total'] / data['ventas_count']).quantize(Decimal('0.01')) if data['ventas_count'] else Decimal('0.00'))
        ranking_cajeros.append({'usuario': data['usuario'], 'ventas_count': data['ventas_count'], 'ingreso_total': _clp_int(data['ingreso_total']), 'ticket_promedio': _clp_int(ticket_prom)})
    ...
    for m in range(5, -1, -1):
        ...
        gains_wave.append(_clp_int(to_clp_pesos(ganancia_neta_mes)))
    ...
    ingreso_prev = to_clp_pesos(ventas_prev.aggregate(t=Sum('total'))['t'] or Decimal('0.00'))
    ...
    cmv_prev = to_clp_pesos(Decimal(str(cmv_prev_val)))
    ingreso_sin_iva_prev = to_clp_pesos((ingreso_prev / Decimal('1.19')).quantize(Decimal('0.01')) if ingreso_prev else Decimal('0.00'))
    costo_prev_net = to_clp_pesos((cmv_prev / Decimal('1.19')).quantize(Decimal('0.01')) if cmv_prev else Decimal('0.00'))
    ganancia_neta_prev = to_clp_pesos(ingreso_sin_iva_prev - costo_prev_net)
    ...
    return {
        ...
        'ingreso_delta': to_clp_pesos(ingreso_delta),
        'ingreso_pct': ingreso_pct,
        'ganancia_neta_delta': to_clp_pesos(ganancia_neta_delta),
